  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="font-display text-3xl text-primary">RelatÃ³rios do Dia</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {isLoading ? (
          <Card>Carregando relatÃ³rios...</Card>
        ) : relatorios.length === 0 ? (
          <Card>Nenhum relatÃ³rio encontrado.</Card>
        ) : (
          relatorios.map((rel) => (
            <Card key={rel.id} className="flex flex-col gap-3">
              <div>
                ...
                <p className="text-xs text-muted">
                  Turno: {formatDateTime(rel.turno_iniciado_em)} â†’{' '}
                  {formatDateTime(rel.turno_encerrado_em)}
                </p>
              </div>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>AlocaÃ§Ãµes: {rel.total_alocacoes}</div>
                <div>Colaboradores: {rel.total_colaboradores}</div>
                <div>CafÃ©s: {rel.total_cafes}</div>
