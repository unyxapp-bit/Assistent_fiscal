  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="font-display text-3xl text-primary">Relat?rios do Dia</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {isLoading ? (
          <Card>Carregando relat?rios...</Card>
        ) : relatorios.length === 0 ? (
          <Card>Nenhum relat?rio encontrado.</Card>
        ) : (
          relatorios.map((rel) => (
            <Card key={rel.id} className="flex flex-col gap-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>Aloca??es: {rel.total_alocacoes}</div>
                <div>Colaboradores: {rel.total_colaboradores}</div>
                <div>Caf?s: {rel.total_cafes}</div>
