  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="font-display text-3xl text-primary">Notifica??es</h1>
        <p className="text-sm text-muted mt-2">
          Alertas recentes do sistema e eventos cr?ticos do turno.
        </p>
      </div>

      <Card>
        {isLoading ? (
          <p className="text-sm text-muted">Carregando notifica??es...</p>
        ) : eventos.length === 0 ? (
          <p className="text-sm text-muted">Nenhuma notifica??o dispon?vel.</p>
        ) : (
